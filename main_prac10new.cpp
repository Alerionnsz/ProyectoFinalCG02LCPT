...
//*************				Práctica 10  Grupo_02 Ejercicio de clase						******//
//************************************************************//
//************************************************************//
...
void carrusel() {
	...
	glColor3f(1.0, 1.0, 1.0);
	glutSolidCone(3.0, 0.80, rebanadas, sas);
	glRotatef(180, 0, 1, 0);
	glutSolidCone(3.0, 0, rebanadas, sas);
	...
	for (i = 0, c = 0, x = 0, j = -1; i < 10; i++, j += -0.1)
		...
		fig1.esfera(0.3, rebanadas, sas, esfera.GLindex);
		glEnable(GL_LIGHTING);
		glTranslatef(0.3, 0, -0.3);
		fig1.esfera(0.2, rebanadas, sas,0);
		glTranslated(-0.3, 0, 0.5);
		glRotatef(-135, 0, 1, 0);
		glDisable(GL_LIGHTING);
		glutSolidCone(0.1, 0.7, rebanadas, sas);
		glEnable(GL_LIGHTING);
		glTranslatef(0, -0.2, 0);
		fig1.esfera(0.1, rebanadas, sas,esfera.GLindex);
	...
	glTranslatef(0, 0, 3);
	glRotatef(-180, 0, 1, 0);
	glutSolidCone(3.0, 0.1, rebanadas, sas);
	glTranslatef(0, 0, -0.5);
	glutSolidCone(0.9, 4.3, rebanadas, sas);
	glPopMatrix();
	glColor3f(1, 1, 1);
...
void animacion()
{
	...
	if (g_fanimacion) {
		...
		if (movKit <= -130 && dir==false)
			movKit += 0.2;
			dir = true;
			if (movKit <= 130 && dir == true)
				movKit -= 0.2;
			dir = false;
